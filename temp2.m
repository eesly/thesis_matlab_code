...
% step = floor(pN/avN/4);
% set(gca,'FontSize',14);
% plot(ADCPpara.ranges(1:25)./10,avEI(step*4,1:25),'linewidth',2,...
%     'MarkerFaceColor',[0.16 0.38 0.27],'Marker','square','Color',[1 0 0]);hold on;
% plot(ADCPpara.ranges(1:24)./10,avEI(step*3,1:24),'linewidth',2,...
%     'MarkerFaceColor',[1 1 0],'Marker','o','Color',[0 0 1]);hold on;
% plot(ADCPpara.ranges(1:21)./10,avEI(step*2,1:21),'linewidth',2,...
%     'MarkerFaceColor',[0 0 1],'Marker','v','Color',[0 0 0]);hold on;
% plot(ADCPpara.ranges(1:20)./10,avEI(step*1,1:20),'linewidth',2,...
%     'MarkerFaceColor',[1 0 0],'MarkerSize',10,'Marker','*',...
%     'LineStyle','--','Color',[0.043 0.51 0.78]);hold off;
% xlim([min(ADCPpara.ranges(1:25)./10) max(ADCPpara.ranges(1:25)./10)]);
% % ylim([129 149]);
% xlabel('Depth(m)');%ylabel('Backscatter Strength(dB ref 1uPa)');
...
%     Sv(i,:) = avEI(i,:) + 2*rw*depth' + 20*log10(depth') - 216  - 10*log10(Vrec1');
end
...
set(gca,'FontSize',14);
step = floor(pN/avN/4);
plot(ADCPpara.ranges(1:20)./10,SSC(step*4,1:20),'linewidth',2,...
    'MarkerFaceColor',[0.16 0.38 0.27],'Marker','square','Color',[1 0 0]);hold on;
plot(ADCPpara.ranges(1:21)./10,SSC(step*3,1:21),'linewidth',2,...
    'MarkerFaceColor',[1 1 0],'Marker','o','Color',[0 0 1]);hold on;
plot(ADCPpara.ranges(1:21)./10,SSC(step*2,1:21),'linewidth',2,...
    'MarkerFaceColor',[0 0 1],'Marker','v','Color',[0 0 0]);hold on;
plot(ADCPpara.ranges(1:20)./10,SSC(step*1,1:20),'linewidth',2,...
    'MarkerFaceColor',[1 0 0],'MarkerSize',10,'Marker','*',...
    'LineStyle','--','Color',[0.043 0.51 0.78]);hold off;
xlim([min(ADCPpara.ranges./10) max(ADCPpara.ranges(1:21)./10)]);
...
s41 = reshape(EL(4,1,t4),1,length(t4));
s42 = reshape(EL(4,2,t4),1,length(t4));
s43 = reshape(EL(4,3,t4),1,length(t4));
...
annotation(figure1,'textbox',...
    ...
    'FontSize',12,...
    'FontName','Arial Rounded MT Bold',...
    'String',{['R =' num2str(R4-0.01)]},...
    'FitBoxToText','on');
%相关系数
% s = reshape(EL(1,2,1:20),1,20);
% R1 = sum( (s - mean(s)).*(avEI(step*1,1:20) - mean(avEI(step*1,1:20))) )./...
%     sqrt( sum((s - mean(s)).^2) )./...
%     sqrt( sum((avEI(step*1,1:20) - mean(avEI(step*1,1:20))).^2) );
% 
% s = reshape(EL(2,2,1:21),1,21);
% R2 = sum( (s - mean(s)).*(avEI(step*2,1:21) - mean(avEI(step*2,1:21))) )./...
%     sqrt( sum((s - mean(s)).^2) )./...
%     sqrt( sum((avEI(step*2,1:21) - mean(avEI(step*2,1:21))).^2) );
% 
% s = reshape(EL(3,2,1:24),1,24);
% R3 = sum( (s - mean(s)).*(avEI(step*3,1:24) - mean(avEI(step*3,1:24))) )./...
%     sqrt( sum((s - mean(s)).^2) )./...
%     sqrt( sum((avEI(step*3,1:24) - mean(avEI(step*3,1:24))).^2) );
% 
% s = reshape(EL(4,2,1:25),1,25);
% R4 = sum( (s - mean(s)).*(avEI(step*4,1:25) - mean(avEI(step*4,1:25))) )./...
%     sqrt( sum((s - mean(s)).^2) )./...
%     sqrt( sum((avEI(step*4,1:25) - mean(avEI(step*4,1:25))).^2) );
% 
% figure1 = figure;
% boardW = 0.1;
% gap = 0.01;
...
% set(gca,'FontSize',14);
% step = floor(pN/avN/4);
% plot(ADCPpara.ranges(1:20)./10,reshape(EL(1,2,1:20),1,20),'linewidth',2,...
%     'MarkerFaceColor',[1 0 0],'MarkerSize',8,'Marker','*',...
%     'LineStyle','--','Color',[1 0 0]);hold on;
% plot(ADCPpara.ranges(1:20)./10,avEI(step*1,1:20),'linewidth',2,...
%     'MarkerFaceColor',[0.34 0.20 0.32],'MarkerSize',3,'Marker','o',...
%     'LineStyle','--','Color',[0 0 0]);hold off;
% xlim([min(ADCPpara.ranges./10) max(ADCPpara.ranges(1:20)./10)]);
% ylabel('Backscatter Strength');set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[]);
% legend('simulated','measured');
% grid on;
% 
% subplot('Position',[boardW+sizeL + gap boardW+sizeL + gap  sizeL sizeL]) 
% set(gca,'FontSize',14);
% plot(ADCPpara.ranges(1:21)./10,reshape(EL(2,2,1:21),1,21),'linewidth',2,...
%     'MarkerFaceColor',[1 0 0],'MarkerSize',8,'Marker','*',...
%     'LineStyle','--','Color',[1 0 0]);hold on;
% plot(ADCPpara.ranges(1:21)./10,avEI(step*2,1:21),'linewidth',2,...
%     'MarkerFaceColor',[0.34 0.20 0.32],'MarkerSize',3,'Marker','o',...
%     'LineStyle','--','Color',[0 0 0]);hold off;
% xlim([min(ADCPpara.ranges./10) max(ADCPpara.ranges(1:21)./10)]);
% set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[]);legend('simulated','measured');
% grid on;
% 
% subplot('Position',[boardW boardW  sizeL sizeL]) 
% set(gca,'FontSize',14);
% plot(ADCPpara.ranges(1:24)./10,reshape(EL(3,2,1:24),1,24),'linewidth',2,...
%     'MarkerFaceColor',[1 0 0],'MarkerSize',8,'Marker','*',...
%     'LineStyle','--','Color',[1 0 0]);hold on;
% plot(ADCPpara.ranges(1:24)./10,avEI(step*3,1:24),'linewidth',2,...
%     'MarkerFaceColor',[0.34 0.20 0.32],'MarkerSize',3,'Marker','o',...
%     'LineStyle','--','Color',[0 0 0]);hold off;
% xlim([min(ADCPpara.ranges./10) max(ADCPpara.ranges(1:24)./10)]);
% xlabel('Depth');legend('simulated','measured');
% set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[]);
% grid on;
% 
% subplot('Position',[boardW+sizeL + gap boardW  sizeL sizeL]) 
% set(gca,'FontSize',14);
% plot(ADCPpara.ranges(1:25)./10,reshape(EL(4,2,1:25),1,25),'linewidth',2,...
%     'MarkerFaceColor',[1 0 0],'MarkerSize',8,'Marker','*',...
%     'LineStyle','--','Color',[1 0 0]);hold on;
% plot(ADCPpara.ranges(1:25)./10,avEI(step*4,1:25),'linewidth',2,...
%     'MarkerFaceColor',[0.34 0.20 0.32],'MarkerSize',3,'Marker','o',...
%     'LineStyle','--','Color',[0 0 0]);hold off;legend('simulated','measured');
% xlim([min(ADCPpara.ranges./10) max(ADCPpara.ranges(1:25)./10)]);
% xlabel('Depth');set(gca,'YTickLabel',[]);set(gca,'XTickLabel',[]);
% grid on;
% 
% annotation(figure1,'textbox',...
%     [0.2 0.69 0.22 0.04],...
